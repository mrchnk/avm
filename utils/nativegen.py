...
TYPEMAP_ATOM_TO_GCREF = {
    ...
    CTYPE_OBJECT: lambda val, t: "GCRef<%s>((%s*)(avmplus::AvmCore::atomToScriptObject(%s)))" % (
        t.fqcppname(), t.fqcppname(), val),
...
def warn_notwriteable(file):
    if path.exists(file) and not os.stat(file).st_mode & stat.S_IWUSR:
...
def stripVersion(uri):
    if len(uri) > 0:
        ...
        if cc >= MIN_API_MARK and cc <= MAX_API_MARK:
            return cc - MIN_API_MARK, uri[0:-1]
    return -1, uri
...
class MethodInfo(MemberInfo):
    ...
    def assign_names(self, traits, prefix):
        ...
        if self.name.ns.srcname is not None:
...
class NativeInfo:
    ...
    def parse_one_nativeinfo(self, attrs, is_vm_builtin):
        ...
        for k in list(attrs.keys()):
            ...
            if k == "script":
                raise Error(
                    "native scripts are no longer supported; please use a native class instead and wrap with AS3 code as necessary.")
        ...
        if (self.traits.cpp_name_comps is None) and (self.itraits.cpp_name_comps is None):
            ...
            if not (
                    self.construct == "native" or self.traits.cpp_friend_classes is not None or self.class_gc_exact is not None or self.instance_gc_exact is not None):
    ...
    def set_friend(self, name):
        if self.traits.cpp_friend_classes is not None:
            raise Error("native(friend) may not be specified multiple times for the same class: %s %s" % (
                self.traits.fqcppname(), name))
        self.traits.cpp_friend_classes = name.split(',')
        self.itraits.cpp_friend_classes = name.split(',')

    def set_class(self, name):
        if self.traits.cpp_name_comps is not None:
            raise Error("native(cls) may not be specified multiple times for the same class: %s %s" % (
                self.traits.fqcppname(), name))
        self.traits.cpp_name_comps = self.fullyQualifiedCPPClassName(name)

    def set_instance(self, name):
        if self.itraits.cpp_name_comps is not None:
            raise Error("native(instance) may not be specified multiple times for the same class: %s %s" % (
                self.itraits.fqcppname(), name))
    ...
    def set_instancebase(self, name):
        if self.instancebase_name is not None:
            raise Error("native(instancebase) may not be specified multiple times for the same class: %s %s" % (
    ...
    def set_classGC(self, gc):
        if self.class_gc_exact is not None:
            raise Error("native(classgc) may not be specified multiple times for the same class: %s %s" % (
                self.traits.fqcppname(), gc))
        if gc == "exact":
            self.class_gc_exact = True
        elif gc == "conservative":
            self.class_gc_exact = False
        else:
            raise Error("native(classgc) can only be specified as 'exact' or 'conservative': %s %s" % (
                self.traits.fqcppname(), gc))

    def set_instanceGC(self, gc):
        if self.instance_gc_exact is not None:
            raise Error("native(instancegc) may not be specified multiple times for the same class: %s %s" % (
                self.itraits.fqcppname(), gc))
        if gc == "exact":
            self.instance_gc_exact = True
        elif gc == "conservative":
            self.instance_gc_exact = False
        else:
            raise Error("native(instancegc) can only be specified as 'exact' or 'conservative': %s %s" % (
                self.itraits.fqcppname(), gc))
    ...
    def parse_native_info(t, is_vm_builtin):
        ...
        if itraits.is_interface:
            if t.cpp_name_comps is not None or itraits.cpp_name_comps is not None:
            ...
            if t.cpp_name_comps is None and itraits.cpp_name_comps is not None:
                raise Error("It is not legal to specify native(instance) without native(cls)")

            if t.cpp_name_comps is None:
                t.cpp_name_comps = ni.fullyQualifiedCPPClassName(itraits.name.name + "Class")
                t.is_synthetic = True
                # if gc metadata not specified, assume we want it exact
                if ni.class_gc_exact is None:
                    ni.class_gc_exact = True

            if itraits.cpp_name_comps is None:
                if str(t.name) == "Object$":
                    ...
                    itraits.is_synthetic = True
                    # if gc metadata not specified, assume we want it exact
                    if ni.instance_gc_exact is None:
        ...
        if str(t.name) == "Object$":
            ...
            assert t.cpp_name_comps is not None

        elif t.cpp_name_comps is not None:
        ...
        assert itraits is not None
        itraits.fqinstancebase_name = ni.instancebase_name
        if ni.method_map_name is not None:
...
class ByteArray:
    ...
    def readUTF8(self):
        ...
        return self.readBytes(lenbytes).decode('utf-8')
                        ...
                        for md in m.metadata:
                            if md.name == "native":
                                if "script" in md.attrs:
                                    raise Error(
                                        "native(script) is no longer supported; please use a native(\"function-name\") instead: " + str(
                                            m.name))
                                if len(md.attrs) != 1 or "" not in md.attrs:
                                    raise Error(
                                        "native(\"function-name\") is the only form supported here" + str(m.name))
                                if not m.isNative():
                                    raise Error(
                                        "native(\"function-name\") can only be used on native functions" + str(m.name))
                                m.receiver = None
                                m.native_method_name = md.attrs[""]  # override
                                m.native_id_name = "native_script_function_" + ns_prefix(m.name.ns, False) + m.name.name

    def assign_names(self, traits, prefix):
        if traits.init is not None:
...
class AbcThunkGen:
    ...
    def emitAOT(self, out, name):
        ...
        traits = [t for t in self.abc.classes + self.abc.instances if
                  (not (t.itraits is not None and t.itraits.is_interface)) and \
                  (not t.is_interface) and \
                  (not t.is_synthetic) and \
                  (t.fqcppname() != "double") and \
                  ((t.ctype == CTYPE_OBJECT) or (t.fqcppname() == BASE_INSTANCE_NAME) or (
                          t.fqcppname() == BASE_CLASS_NAME))]
        ...
        for i in range(0, len(glueClasses)):
            out.println('const char* const n_%(count)u; %(glueClass)s* const m_%(count)u;' % {'count': i, 'glueClass':
                glueClasses[i][0]})
    ...
    def emit_h(self, out, name):
        ...
        for i in range(0, len(self.abc.methods)):
            ...
            if m.native_id_name is not None:
                assert (m.id == i)
    ...
    def emit_cpp(self, out, name):
        ...
        for i in range(0, len(self.abc.methods)):
            m = self.abc.methods[i]
            if m.isNative() and (m.receiver is None or not m.receiver.is_interface):
                assert (m.native_method_name is not None)
                assert (m.native_id_name is not None)
                if m.receiver is None:
                    ...
                    out.println(
                        "%s(%s, %s::%s)" % (nmout, m.native_id_name, m.receiver.method_map_name, m.native_method_name))
        out.indent -= 1
    ...
    def forwardDeclareGlueClasses(self, out_h):
        ...
        for i in range(0, len(self.abc.classes)):
            ...
            if c.itraits.is_gc_exact:
                ...
                while base is not None and base.fqcppname() != BASE_INSTANCE_NAME:
                    if not base.is_gc_exact:
                        if c.itraits.is_synthetic:
                            raise Error(
                                "class %s is implicitly assumed to be exact-gc but its ancestor %s is not; this is not supported" % (
                                    c.itraits.fqcppname(), base.fqcppname()))
                        else:
                            raise Error(
                                "class %s is explicitly marked as exact-gc but its ancestor %s is not; this is not supported" % (
                                    c.itraits.fqcppname(), base.fqcppname()))
    ...
    def emitGlueClassManifest(self, out):
        ...
        out.println(
            "REALLY_INLINE static %s* create(avmplus::ScriptEnv* e) { return new (MMgc::GC::GetGC(e), MMgc::kExact, sizeof(ClassClosure*)*%d) %s(e); }" % (
                man_name, len(names) - 1, man_name))
        out.indent -= 1
        out.println("public:")
        out.indent += 1
        for as3name, cppname, clsid in names:
            # We can't use static_cast<> because the subclass is only forward-declared at this point
            out.println("REALLY_INLINE GCRef<%s> get_%s() { return (%s*)(lazyInitClass(%s)); }" % (
                cppname, as3name, cppname, clsid))
        out.indent -= 1
        out.println("};")
    ...
    def sortSlots(self, t):
        ...
        sortedSlots = sorted(t.slots, key=cmp_to_key(lambda x, y: self.cmpSlots(x, y, slotsTypeInfo)))
            ...
            if numTracedSlots > 0:
                ...
                for slot in sortedSlots:
                    if slot is not None:
                        ...
                        elif (slotTraits.ctype == CTYPE_STRING) or (slotTraits.ctype == CTYPE_NAMESPACE) or (
                                slotTraits.ctype == CTYPE_OBJECT):
    ...
    def emitConstructStubs(self, out, t, sortedSlots, slotsTypeInfo):
        ...
        if t.itraits is not None:
            out.println("/*static*/ %s* FASTCALL %s::createClassClosure(avmplus::VTable* cvtable)" % (
                BASE_CLASS_NAME, t.fqcppname()))
            out.println("{")
            out.indent += 1
            if t.is_gc_exact:
                out.println("cvtable->ivtable->createInstanceProc = %s;" % t.createInstanceProcName)
                out.println(
                    "ClassClosure* const cc = new (cvtable->gc(), MMgc::kExact, cvtable->getExtraSize()) %s(cvtable);" % t.fqcppname())
            else:
                out.println("cvtable->ivtable->createInstanceProc = %s;" % t.createInstanceProcName)
                out.println(
                    "ClassClosure* const cc = new (cvtable->gc(), cvtable->getExtraSize()) %s(cvtable);" % t.fqcppname())
            out.println(
                "AvmThunk_DEBUG_ONLY( %s::SlotOffsetsAndAsserts::check_%s(cc->traits(), cc->traits()->itraits); )" % (
                    opts.nativeIDNS, to_cname(t.fqcppname())))
            out.println("return cc;")
            out.indent -= 1
            out.println("}")

            if t.has_custom_createInstanceProc:
                out.println("/*static*/ avmplus::ScriptObject* FASTCALL %s::createInstanceProc(%s* cls)" % (
                    t.fqcppname(), BASE_CLASS_NAME))
                out.println("{")
                out.indent += 1
                if t.has_pre_create_check:
                ...
                if t.itraits.is_gc_exact:
                    out.println(
                        "return new (cls->gc(), MMgc::kExact, cls->getExtraSize()) %s(cls->ivtable(), cls->prototypePtr());" % (
                            t.itraits.fqcppname()))
                else:
                    out.println(
                        "return new (cls->gc(), cls->getExtraSize()) %s(cls->ivtable(), cls->prototypePtr());" % (
                            t.itraits.fqcppname()))
                out.indent -= 1
                out.println("}")

        if not t.has_construct_method_override:
            if t.fqinstancebase_name is not None:
                baseclassname = t.fqinstancebase_name
            else:
                baseclassname = self.lookupTraits(t.base).fqcppname()
            out.println(
                "AvmThunk_DEBUG_ONLY( avmplus::Atom %s::construct(int argc, avmplus::Atom* argv) { return %s::construct(argc, argv); } )" % (
                    t.fqcppname(), baseclassname))
    ...
    def shouldEmitConstructObject(self, t):
        ...
        return t.itraits is not None and \
    ...
    def emitConstructObjectDeclaration(self, out, t, args):
        ...
        arglist = ', '.join(
            ["%s %s" % (argt_arg_typedef_argname4[1], argt_arg_typedef_argname4[2]) for argt_arg_typedef_argname4 in
             args[1:]])
    ...
    def emitMethodWrappers(self, out, t):
        if self.shouldEmitConstructObject(t):
            ...
            for i in range(0, t.itraits.init.optional_count + 1):
                arglist = ', '.join(["%s %s" % (argt_arg_typedef_argname2[1], argt_arg_typedef_argname2[2]) for
                                     argt_arg_typedef_argname2 in args[1:]])
                ...
                arglist = ', '.join(
                    [TYPEMAP_TO_ATOM[argt_arg_typedef_argname3[0].ctype](argt_arg_typedef_argname3[2]) for
                     argt_arg_typedef_argname3 in args])
                out.println("avmplus::Atom args[%d] = { %s };" % (len(args), arglist))
                if t.construct == "native":
                    out.println(
                        "avmplus::Atom const result = this->construct_native(%s::createInstanceProc, %d, args);" % (
                            t.fqcppname(), len(args) - 1))
                else:
        ...
        for v in ["public", "protected", "private"]:
            ...
            cpp_vis[v].sort(key=lambda mi: "%s%s" % (prefixes[mi.kind], mi.name.name))
            for mi in cpp_vis[v]:
                ...
                for i in range(0, mi.optional_count + 1):
                    arglist = ', '.join(["%s %s" % (argt_arg_typedef_argname[1], argt_arg_typedef_argname[2]) for
                                         argt_arg_typedef_argname in args[1:]])
                    # "inline" rather than "REALLY_INLINE" -- it's not essential to inline
                    # this, so let the compiler decide to deinline if it so chooses
                    ...
                    out.println(
                        "inline %s %s%s(%s%s)" % (ret_typedef, prefixes[mi.kind], mi.name.name, pool_string, arglist))
                    out.println("{")
                    out.indent += 1
                    # bah, does Python not have an equivalent to JS Array.some?
                    needcore = False
                    ...
                    if needcore:
                        ...
                        out.println("avmplus::AvmCore* const core = ((avmplus::AvmCore*)(this->core()));")
                    arglist = ', '.join(
                        [TYPEMAP_TO_ATOM[argt_arg_typedef_argname1[0].ctype](argt_arg_typedef_argname1[2]) for
                         argt_arg_typedef_argname1 in args])
                    argc = len(args) - 1
                    if t.is_interface:
                        ...
                        out.println(
                            "const avmplus::Multiname* const mn = pool->precomputedMultiname(%d);" % mi.name_index)
                        if mi.kind == TRAIT_Getter:
                            ...
                            out.println("this->toplevel()->setproperty(this->atom(), mn, %s, this->vtable);" % (
                                TYPEMAP_TO_ATOM[argt.ctype](argname)))
                            out.println("avmplus::Atom const result = undefinedAtom;")
                        else:
                            out.println("avmplus::Atom args[%d] = { %s };" % (len(args), arglist))
                            out.println(
                                "avmplus::Atom const result = this->toplevel()->callproperty(this->atom(), mn, %d, args, this->vtable);" % argc)
                    else:
                        idx = mi.vtable_index
                        if mi.kind == TRAIT_Setter:
                            idx += 1
        ...
        if t.interfaces is not None and len(t.interfaces) > 0:
            out.println("public:")
            out.indent += 1
    ...
    def emitSlotDeclarations(self, out, t, sortedSlots, slotsTypeInfo, closingSemi):
        ...
        if t.cpp_friend_classes is not None:
        ...
        if len(t.slots) > 0:
            for slot in sortedSlots:
                ...
                if slotCType == CTYPE_BOOLEAN:
                    suffix = " != 0"
                out.println("REALLY_INLINE %s get_%s() const { return %s.m_%s%s; }" % (
                    slotRetType, slotAccessorName, t.slotsInstanceName, slotMemberName, suffix))
                if slot.kind == TRAIT_Slot:
                    out.println("REALLY_INLINE void set_%s(%s newVal) { %s.m_%s = newVal; }" % (
                        slotAccessorName, slotArgType, t.slotsInstanceName, slotMemberName))
                elif slot.kind == TRAIT_Const:
                    out.println("REALLY_INLINE void setconst_%s(%s newVal) { %s.m_%s = newVal; }" % (
                        slotAccessorName, slotArgType, t.slotsInstanceName, slotMemberName))
                out.indent -= 1
            out.println("private:")
            out.indent += 1
    ...
    def emitOneSyntheticClass(self, out, t):
        ...
        either_is_interface = (t.is_interface or (t.itraits is not None and t.itraits.is_interface))
        ...
        out.indent += 1
        if t.itraits is not None:
            out.println("inline explicit %s(VTable* cvtable) : %s(cvtable) { createVanillaPrototype(); }" % (
                t.cppname(), baseclassname))
        else:
            out.println("friend class %s;" % t.ctraits.fqcppname())
            out.println(
                "REALLY_INLINE explicit %s(VTable* ivtable, ScriptObject* delegate) : %s(ivtable, delegate) {}" % (
                    t.cppname(), baseclassname))
        out.indent -= 1
        out.println("private:")
        out.indent += 1
    ...
    def emitSyntheticClassGcTrace(self, out, t, sortedSlots, slotsTypeInfo):
        ...
        if numTracedSlots > 0:
            ...
            for slot in sortedSlots:
                if slot is not None:
                    ...
                    if slotTraits.ctype in [CTYPE_ATOM, CTYPE_STRING, CTYPE_NAMESPACE, CTYPE_OBJECT]:
                        out.println(
                            'offsetof(%s, m_slots_%s.m_%s),' % (t.fqcppname(), t.cppname(), to_cname(slot.name)))
                else:
                    out.println(
                        'offsetof(%s, m_slots_%s.__anonymous_slot_%u),' % (t.fqcppname(), t.cppname(), anonCount))
                    anonCount = anonCount + 1
            out.println("0};")
            out.indent -= 1
        ...
        if base is not None:
            out.println(
                "if ((result = %s::gcTraceOffsetIsTraced(off)) != MMgc::kOffsetNotFound) return result;" % base.fqcppname())
        if numTracedSlots > 0:
    ...
    def printStructAsserts(self, out, abc):
        ...
        out.println('enum {')
        out.indent += 1
        visited_native_classes = set()
        for i in range(0, len(abc.classes)):
            c = abc.classes[i]
            if c.itraits.is_interface:
                continue
            if c.fqcppname() not in visited_native_classes:
                visited_native_classes.add(c.fqcppname())
                out.println('kSlotsOffset_%s = %s,' % (to_cname(c.fqcppname()), c.cpp_offsetof_slots()))
            if c.itraits.fqcppname() not in visited_native_classes:
                visited_native_classes.add(c.itraits.fqcppname())
        ...
        for i in range(0, len(abc.classes)):
            ...
            if c.itraits.is_interface:
                continue
            out.println(
                'REALLY_INLINE void SlotOffsetsAndAsserts::check_%s(Traits* ctraits, Traits* itraits)' % to_cname(
                    c.fqcppname()))
            out.println('{')
            out.indent += 1
    ...
    def printStructAssertsForTraits(self, out, t, traitsVarName):
        if len(t.slots) > 0:
            out.println('MMGC_STATIC_ASSERT(offsetof(%s, %s) == kSlotsOffset_%s);' % (
                t.fqcppname(), t.slotsInstanceName, to_cname(t.fqcppname())))
            ...
            for slot in t.slots:
                if slot is not None:
                    out.println('AvmAssert(getSlotOffset(%s, %u) == (offsetof(%s, %s) + offsetof(%s, m_%s)));'
                                % (traitsVarName, self.namesDict[id(slot.name)], t.fqcppname(), t.slotsInstanceName,
                                   t.slotsStructName, to_cname(slot.name)))
            for mi in t.tmethods:
    ...
    def thunkInfo(self, m):
        ...
        sampler_decl = "%s %s_sampler_thunk(MethodEnv* env, uint32_t argc, Atom* argv)" % (
            rettype_str, m.native_id_name)
    ...
    def emitThunkBody(self, out, receiver, m):
        ...
        for i in range(1, len(argtraits)):
            ...
            if i > param_count - optional_count:
                ...
                if dct != arg_ctype:
                    defval = "AvmThunkCoerce_%s_%s(%s)" % (
                        TYPEMAP_ARGTYPE_SUFFIX[dct], TYPEMAP_ARGTYPE_SUFFIX[arg_ctype], defval)
                val = "(argc < " + str(i) + " ? " + defval + " : " + val + ")"
                if arg_ctype == CTYPE_OBJECT:
                    coercename = argtraits[i].fqcppname()
                    if coercename is not None:
    ...
    def find_override_base(self, mi):
        if mi.override and mi.receiver.base is not None:
...
def main():
    ...
    parser.add_option("-e", "--externmethodandclassetables", action="store_true", default=False,
                      help="generate extern decls for method and class tables")
    parser.add_option("--native-id-namespace", dest="nativeIDNS", default="avmplus::NativeID",
                      help="Specifies the C++ namespace in which all generated should be in.")
    parser.add_option("--root-implementation-namespace", dest="rootImplNS", default="avmplus",
                      help="Specifies the C++ namespace in under which all implementation classes can be found.")
    ...
    for file in files:
        ...
        abc_script_name = path.splitext(path.basename(file))[0]
        abc = Abc(data, abc_script_name)
        ngen.addAbc(abc)
        output = opts.output or path.dirname(file)
        abc_gen_name = path.join(output, abc_script_name)

    if abc:
        hfile = None
        clsfile = None
        cppfile = None
        if warn_notwriteable(abc_gen_name + ".h") | warn_notwriteable(abc_gen_name + "-classes.hh") | warn_notwriteable(
                abc_gen_name + ".cpp"):
            exit(0)  # exit 0 so build will continue
        try:
            hfile = open(abc_gen_name + ".h", "w")
            clsfile = open(abc_gen_name + "-classes.hh", "w")
            cppfile = open(abc_gen_name + ".cpp", "w")
            h = IndentingPrintWriter(hfile)
            cls = IndentingPrintWriter(clsfile)
            c = IndentingPrintWriter(cppfile)
            ngen.emit(abc, abc_script_name, h, cls, c)
