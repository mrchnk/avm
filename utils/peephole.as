...
/* Usage: 
   ...
   to find wopcodes.cpp and there's parameter to specify it.
   Normally you want the input to be "peephole.tbl" + "wopcodes.cpp" and
   the output to be "peephole.cpp":
   
     avmshell peephole.abc -- peephole.tbl wopcodes.cpp peephole.cpp
 */
...
package peephole
{
	...
	var s = [];
	s.push("const uint16_t WordcodeEmitter::toplevel[] = {");
	var i=0;
	while (i < MAXINSTR) {
	    var t = "";
	    var n = "";
	...
	var s = [];
	s.push("bool WordcodeEmitter::commit(uint32_t action)");
	s.push("{");
    ...
    if (System.argv.length != 3) {
        print("Usage: peephole.abc -- peephole.tbl wopcodes.cpp outputfile");
        System.exit(1);
    }

    readOpcodes(System.argv[1]);
